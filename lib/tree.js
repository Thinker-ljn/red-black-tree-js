...
class Tree {
  ...
  find (nodeKey, isInsert = false) {
    if (this.root.isNull) return null

    let currNode = this.root

    while (!currNode.isNull) {
      if (nodeKey < currNode.key) {
        if (isInsert && currNode.left.isNull) break
        currNode = currNode.left
      } else if (currNode.key < nodeKey) {
        if (isInsert && currNode.right.isNull) break
  ...
  remove (nodeKey) {
    ...
    let childNode = !node.left.isNull ? node.left : node.right
  ...
  inorderTraversal (callback, node, deep = 1) {
    ...
    if (node.left && (!node.left.isNull || node.left.graph !== null)) this.inorderTraversal(callback, node.left, deep + 1)
    callback(node, deep)
    this.inorderList.push(node)
    if (node.right && (!node.right.isNull || node.left.graph !== null)) this.inorderTraversal(callback, node.right, deep + 1)
  }
