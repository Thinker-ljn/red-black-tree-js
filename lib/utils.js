...
export function isLeftChild (node) {
  ...
  return parent.left === node || (parent.left.isNull && parent.right !== node)
}

export function isRightChild (node) {
  let parent = node.parent
  return parent.right === node || (parent.right.isNull && parent.left !== node)
}

export function isBlack (node) {
  return node.isNull || node.color === 'black'
}

export function isRed (node) {
  return !node.isNull && node.color === 'red'
}
